...
async function loadTokenMap() {
  try {
    const url = (typeof browser !== "undefined" ? browser.runtime.getURL("token2username.txt") : chrome.runtime.getURL("token2username.txt"));
...
// ------------------ storage 变化监听 ------------------
if (typeof browser !== "undefined" && browser.storage) {
  browser.storage.onChanged.addListener((changes, area) => { if (area === 'local') loadConfig(); });
} else if (typeof chrome !== "undefined" && chrome.storage && chrome.storage.onChanged) {
  chrome.storage.onChanged.addListener((changes, area) => { if (area === 'local') loadConfig(); });
}
...
async function sendToBark(title, body) {
  if (!config.barkKey) { console.warn("Bark Key 未配置，跳过发送"); return { ok:false, code:"no_key" }; }
  ...
  try {
    ...
    if (!res.ok) { console.error("Bark 返回非 200:", res.status); return { ok:false, code: res.status }; }
    recordSend();
    return { ok:true };
  } catch(e){ console.error("Bark 发送异常:", e); return { ok:false, code:"fetch_error", err:String(e) }; }
}
...
function processRetryQueue() {
  ...
  (async () => {
    for (const it of due) {
      if (it.tries >= config.maxRetries) { console.warn("达到最大重试次数，丢弃:", it.key); continue; }
      const res = await sendToBark(it.title, it.body);
      if (!res.ok) { it.tries = (it.tries||0)+1; it.nextTryAt = Date.now()+config.retryIntervalMs; retryQueue.push(it); }
...
function handleForwardNotification(payload) {
  try {
    ...
    if (!isWhitelisted(payload.url)) { console.debug("非白名单域名，忽略:", payload.url); return; }

    cleanRecent();
    const key = `${payload.title}||${payload.body}||${payload.tag}`;
    if (recentMap.has(key)) { console.debug("重复通知（去重）:", key); return; }
    if (!allowSend()) { console.warn("速率限制触发，忽略本次发送"); return; }
    recentMap.set(key, Date.now());

    // --- 新增：解析 URL 得到 token 并映射 username ---
